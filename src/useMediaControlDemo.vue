...
<template>
  ...
  >
    <div class="relative mt-5 overflow-hidden rounded-md bg-black shadow">
      <video
        ref="video"
    ...
    <div class="flex flex-row items-center items-center">
      <button @click="playing = !playing">
        <i v-if="!playing" i-carbon-play inline-block align-middle />
      ...
      <Scrubber v-model="volume" :max="1" class="ml-2 w-32" />
      <div
        class="ml-2 flex flex-1 flex-col text-sm"
      >
  ...
  <pre class="code-block" lang="yaml">{{ JSON.stringify(controls, null, 2) }}</pre>
</template>
